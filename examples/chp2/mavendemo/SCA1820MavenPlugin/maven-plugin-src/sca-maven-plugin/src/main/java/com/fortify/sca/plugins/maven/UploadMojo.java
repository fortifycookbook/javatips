...
public class UploadMojo extends AbstractFortifyClientMojo {
    ...
    protected List<String> buildOptions() {
        ...
        if (!StringUtil.isEmpty(sscApplicationVersionId)) {
            OptionUtil.setOption(options, "-projectVersionID", sscApplicationVersionId);
        } else {
            OptionUtil.setOption(options, "-project", sscApplicationName);
            OptionUtil.setOption(options, "-version", sscApplicationVersion);
        }
