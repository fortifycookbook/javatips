...
public class RarPackagingHandler extends JarPackagingHandler {
    ...
    protected boolean addMainFiles(Commandline commandline) {
        ...
        Plugin warPlugin = currentProject.getPlugin("org.apache.maven.plugins:maven-rar-plugin");
        String warSourceDirPath = PluginUtil.getExecutionConfig(warPlugin, "default-rar", "rarSourceDirectory", DEFAULT_RAR_SRC_DIR);

        if (FileUtil.containsFile(warSourceDirPath)) {
            commandline.createArg().setLine(FileUtil.normalizeFilePath(warSourceDirPath));
            added = true;
            log.info("Rar Source Dir: " + warSourceDirPath);
        } else {
            ...
            if (FileUtil.containsFile(warSourceDir)) {
                ...
                added = true;
                log.info("Rar Source Dir: " + warSourceDir.getAbsolutePath());
            }
        }
